...
impl<T: Iterator<Item=char>> Parser<T> {
    ...
    fn parse(&mut self) -> Result<(Event, Marker), ScanError> {
    ...
    pub fn load<R: MarkedOwnedEventReceiver>(&mut self, recv: &mut R, multi: bool)
        -> Result<(), ScanError> {
        ...
        if self.scanner.stream_ended() {
            // XXX has parsed?
            recv.on_owned_event(Event::StreamEnd, self.scanner.mark());
            return Ok(());
        }
        loop {
            let (ev, mark) = try!(self.parse());
    ...
    fn load_document<R: MarkedOwnedEventReceiver>(&mut self, first_ev: Event, mark: Marker, recv: &mut R)
        -> Result<(), ScanError> {
    ...
    fn load_node<R: MarkedOwnedEventReceiver>(&mut self, first_ev: Event, mark: Marker, recv: &mut R)
        -> Result<(), ScanError> {
        match first_ev {
    ...
    fn load_mapping<R: MarkedOwnedEventReceiver>(&mut self, recv: &mut R)
        -> Result<(), ScanError> {
    ...
    fn load_sequence<R: MarkedOwnedEventReceiver>(&mut self, recv: &mut R)
        -> Result<(), ScanError> {
