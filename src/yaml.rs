...
impl Yaml {
    ...
    pub fn from_str(v: &str) -> Yaml {
        if let Some(end) = v.strip_prefix("0x") {
            if let Ok(i) = i64::from_str_radix(end, 16) {
                return Yaml::Integer(i);
            }
        }
        if let Some(end) = v.strip_prefix("0o") {
            if let Ok(i) = i64::from_str_radix(end, 8) {
                return Yaml::Integer(i);
            }
        }
        if let Some(end) = v.strip_prefix('+') {
            if let Ok(i) = end.parse::<i64>() {
                return Yaml::Integer(i);
            }
        }
        match v {
